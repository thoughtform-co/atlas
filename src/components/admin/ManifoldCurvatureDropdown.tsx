...
interface ManifoldCurvatureDropdownProps {
  value: string;
  onChange: (value: string) => void;
  placeholder?: string;
}

const MANIFOLD_CURVATURE_OPTIONS = ['Stable', 'Moderate', 'Severe', 'Critical'];

export function ManifoldCurvatureDropdown({
  value,
  onChange,
  placeholder = 'Select curvature...',
}: ManifoldCurvatureDropdownProps) {
  ...
  const filteredOptions = isTyping && inputValue
    ? MANIFOLD_CURVATURE_OPTIONS.filter(opt => opt.toLowerCase().includes(inputValue.toLowerCase()))
    : MANIFOLD_CURVATURE_OPTIONS;
